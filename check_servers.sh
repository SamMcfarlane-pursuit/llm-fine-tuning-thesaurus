...
# Check server on port 5001
echo "Checking server on port 5001..."
curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:5001/ > /dev/null
if [ $? -eq 0 ]; then
    echo "✅ Server on port 5001 is running."
else
    echo "❌ Server on port 5001 is not running."
fi
