    ...
    def _load_domain_terms(self):
        ...
        terms = {
            ...
            "lora": {
                ...
                "related": ["peft", "fine-tuning", "adapter layers", "rank", "attention layers", "qlora"],
                "description": "Low-Rank Adaptation - a PEFT method that adds trainable low-rank matrices to model weights."
            },
            "qlora": {
                "synonyms": ["quantized low-rank adaptation", "quantized lora", "4-bit lora"],
                "related": ["lora", "peft", "quantization", "fine-tuning", "4-bit quantization", "nf4", "memory efficiency"],
                "description": "Quantized Low-Rank Adaptation - an extension of LoRA that uses 4-bit quantization to dramatically reduce memory requirements for fine-tuning large language models."
            },
            ...
            "quantization": {
                "synonyms": ["model quantization", "weight quantization", "precision reduction"],
                "related": ["optimization", "inference", "int8", "int4", "fp16", "bf16", "deployment", "qlora", "memory efficiency", "post-training quantization", "quantization-aware training"],
                "description": "The process of reducing the precision of model weights to improve performance and reduce memory usage while maintaining model quality."
            },
            "pruning": {
            ...
            "gradient checkpointing": {
                "synonyms": ["activation checkpointing", "memory checkpointing"],
                "related": ["memory optimization", "training", "backpropagation", "gpu memory", "qlora"],
                "description": "A technique that trades computation for memory by recomputing activations during the backward pass instead of storing them."
            },
            "nf4": {
                "synonyms": ["normalfloat4", "4-bit normal float"],
                "related": ["quantization", "qlora", "4-bit quantization", "bitsandbytes"],
                "description": "A 4-bit data type specifically designed for normally distributed weights in neural networks, used in QLoRA for efficient quantization."
            },
            "gpt": {
                "synonyms": ["generative pre-trained transformer", "gpt-3", "gpt-4"],
                "related": ["llm", "transformer", "decoder", "autoregressive", "fine-tuning"],
                "description": "Generative Pre-trained Transformer - a family of large language models developed by OpenAI that use transformer architecture for text generation."
            }
        }
